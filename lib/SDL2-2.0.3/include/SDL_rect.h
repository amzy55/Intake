...
{
    return (a && b && ((a == b) ||
            ((SDL_fabs(static_cast<double>(a->x) - b->x) <= epsilon) &&
            (SDL_fabs(static_cast<double>(a->y) - b->y) <= epsilon) &&
            (SDL_fabs(static_cast<double>(a->w) - b->w) <= epsilon) &&
            (SDL_fabs(static_cast<double>(a->h) - b->h) <= epsilon))))
            ? SDL_TRUE : SDL_FALSE;
}
